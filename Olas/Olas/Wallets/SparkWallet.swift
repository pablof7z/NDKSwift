import Foundation
import NDKSwift
@preconcurrency import BreezSdkSpark
import BIP39
...
actor SparkWallet: NDKPaymentProvider {
    ...
    private func startEventListener() async {
        guard let sdk = sdk else { return }
        eventListenerId = await sdk.addEventListener(listener: SparkEventHandler(continuation: eventContinuation))
    }
...
struct SparkWalletInfo: Sendable {
...
enum SparkWalletError: LocalizedError {
    case notConnected
...
enum SparkPaymentType: Sendable {
    case send
    case receive
}

enum SparkPaymentStatus: Sendable {
    case pending
    case completed
    case failed
}

struct SparkPayment: Identifiable, Sendable {
...
enum SparkParsedInput: Sendable {
...
struct SparkBolt11Details: Sendable {
...
struct SparkBitcoinAddressDetails: Sendable {
    public let address: String
    public let network: String
    public let amountSats: Int64?
}

struct SparkLnurlPayDetails: Sendable {
...
struct SparkLnurlWithdrawDetails: Sendable {
    public let domain: String
...
struct SparkAddressDetails: Sendable {
    public let address: String
}

struct SparkInvoiceDetails: Sendable {
    public let invoice: String
    public let amountSats: Int64
}
...
struct SparkPreparedPayment: Sendable {
    public let destination: String
    public let amountSats: Int64
    public let feeSats: Int64
