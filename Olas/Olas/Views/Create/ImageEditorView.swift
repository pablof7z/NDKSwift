...
struct ImageEditorView: View {
    ...
    private func updatePreview() async {
        ...
        let result = applyEdits(to: sourceImage, filter: filter, intensity: intensity, adjustments: currentAdjustments)
    ...
    private func finalizeImage() async {
        ...
        if currentRotation != 0 || flipped {
            processed = applyTransform(to: processed ?? sourceImage, rotation: currentRotation, flip: flipped)
        }
