...
extension CashuSwift {
    ...
    public static func restore(mint:MintRepresenting,
                               with seed:String,
                               batchSize:Int = 10) async throws -> [CashuSwiftKeysetRestoreResult] {
        // no need to check validity of seed as function would otherwise crash during first det sec generation
        var results = [CashuSwiftKeysetRestoreResult]()
        for keyset in mint.keysets {
            ...
            let result = CashuSwiftKeysetRestoreResult(keysetID: keyset.keysetID,
    ...
    static func restoreForKeyset(mint:MintRepresenting,
        ...
        while emtpyResponses < emptyRuns {
            ...
            let request = CashuSwiftRestoreRequest(outputs: outputs)
            let response = try await Network.post(url: mint.url.appending(path: "/v1/restore"),
                                                  body: request,
                                                  expected: CashuSwiftRestoreResponse.self)
    ...
    public static func restore(from mint: Mint,
                               with seed: String,
                               batchSize: Int = 50) async throws -> (result: [CashuSwiftKeysetRestoreResult],
                                                                     validDLEQ: Bool) {
