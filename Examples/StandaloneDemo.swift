...
print("")
print("// Modern async API using AsyncSequence")
print("for await event in subscription {")
...
print("")
print("// One-shot fetch API")
print("let events = try await ndk.fetchEvents(filter)")
print("print(\"Fetched \\(events.count) events\")")
...
print("")
print("// Cache is automatically created with NDK")
print("let ndk = try await NDK(relayUrls: [\"wss://relay.damus.io\"])")
print("")
print("// Events are automatically cached during subscriptions")
print("// Query cached events")
print("let cachedEvents = await ndk.cache?.queryEvents(filter) ?? []")
print("")
print("// Cache profiles")
print("try? await ndk.cache?.saveProfile(profile, for: user.pubkey)")
print("let profile = await ndk.cache?.getProfile(for: user.pubkey)")
