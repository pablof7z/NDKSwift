...
// MARK: - Wallet Event Data Source

/// Data source for NIP-60 wallet events
@MainActor
public class WalletEventDataSource: ObservableObject {
    @Published public private(set) var walletEvent: NDKCashuWalletEvent?
    ...
    public init(ndk: NDK, pubkey: String) {
        ...
        // Transform events to wallet events
        Task {
            await observeWalletEvents()
        }
    }
    
    private func observeWalletEvents() async {
        dataSource.$data
            .compactMap { events in
                events.sorted { $0.createdAt > $1.createdAt }.first
            }
            .map { event in
                NDKCashuWalletEvent(event: event)
            }
            .sink { [weak self] walletEvent in
...
// MARK: - Wallet History Data Source

/// Data source for wallet transaction history
@MainActor
public class WalletHistoryDataSource: ObservableObject {
    @Published public private(set) var transactions: [NDKEvent] = []
    @Published public private(set) var isLoading = false
    @Published public private(set) var error: Error?
    ...
    public init(ndk: NDK, pubkey: String) {
        ...
        // Bind data source properties
        Task {
            await bindDataSource()
        }
    }
    
    private func bindDataSource() async {
        dataSource.$data
            .map { events in
...
// MARK: - Nutzap Data Source

/// Data source for nutzap events
@MainActor
public class NutzapDataSource: ObservableObject {
    @Published public private(set) var nutzaps: [NDKEvent] = []
    @Published public private(set) var isLoading = false
    @Published public private(set) var error: Error?
    ...
    public init(ndk: NDK, recipientPubkey: String) {
        self.dataSource = ndk.observe(
            filter: NDKFilter(
                kinds: [10301],
    ...
    private func bindDataSource() async {
        dataSource.$data
            ...
            .sink { [weak self] nutzaps in
                self?.nutzaps = nutzaps
            }
            .store(in: &cancellables)
