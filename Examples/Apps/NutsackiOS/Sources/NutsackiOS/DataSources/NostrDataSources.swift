...
// MARK: - Contact List Data Source

/// Data source for user's contact list
@MainActor
public class ContactListDataSource: ObservableObject {
    @Published public private(set) var contactPubkeys: Set<String> = []
    @Published public private(set) var relayList: [String: Set<NDKRelayAccess>] = [:]
    ...
    public init(ndk: NDK, pubkey: String) {
        ...
        Task {
            await observeContactList()
        }
    }
    
    private func observeContactList() async {
        dataSource.$data
            .compactMap { events in
                events.sorted { $0.createdAt > $1.createdAt }.first
            }
            .sink { [weak self] event in
                self?.parseContactList(from: event)
            }
    ...
    private func parseContactList(from event: NDKEvent) {
        // Parse contact pubkeys from 'p' tags
        let contacts = event.tags
            .filter { $0.first == "p" }
            .compactMap { $0.dropFirst().first }
        self.contactPubkeys = Set(contacts)
        
        // Parse relay list from content
        if let data = event.content.data(using: .utf8),
           let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
            var relayDict: [String: Set<NDKRelayAccess>] = [:]
            
            for (url, value) in json {
                if let markerDict = value as? [String: Bool] {
                    var accessSet: Set<NDKRelayAccess> = []
                    if markerDict["read"] == true {
                        accessSet.insert(.read)
                    }
                    if markerDict["write"] == true {
                        accessSet.insert(.write)
                    }
                    if !accessSet.isEmpty {
                        relayDict[url] = accessSet
                    }
                }
            }
            
            self.relayList = relayDict
        }
    }
}
