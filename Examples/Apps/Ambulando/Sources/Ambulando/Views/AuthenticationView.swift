...
struct FootprintShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        // Main foot body (heel to ball)
        path.move(to: CGPoint(x: width * 0.45, y: height * 0.95)) // heel bottom
        path.addCurve(
            to: CGPoint(x: width * 0.25, y: height * 0.45), // outer arch
            control1: CGPoint(x: width * 0.15, y: height * 0.85),
            control2: CGPoint(x: width * 0.1, y: height * 0.6)
        )
        path.addCurve(
            to: CGPoint(x: width * 0.35, y: height * 0.25), // ball outer
            control1: CGPoint(x: width * 0.25, y: height * 0.35),
            control2: CGPoint(x: width * 0.3, y: height * 0.28)
        )
        
        // Big toe
        path.addCurve(
            to: CGPoint(x: width * 0.5, y: height * 0.05), // big toe top
            control1: CGPoint(x: width * 0.38, y: height * 0.15),
            control2: CGPoint(x: width * 0.45, y: height * 0.08)
        )
        path.addCurve(
            to: CGPoint(x: width * 0.65, y: height * 0.25), // big toe inner
        ...
        // Inner foot
        path.addCurve(
            to: CGPoint(x: width * 0.75, y: height * 0.65), // inner arch
            control1: CGPoint(x: width * 0.7, y: height * 0.35),
            control2: CGPoint(x: width * 0.75, y: height * 0.5)
        )
        path.addCurve(
            to: CGPoint(x: width * 0.55, y: height * 0.95), // heel inner
        ...
        // Add small toes
        for i in 0..<4 {
            let toeX = width * (0.3 - Double(i) * 0.06)
            let toeY = height * (0.15 + Double(i) * 0.03)
            let toeSize = width * (0.08 - Double(i) * 0.01)
