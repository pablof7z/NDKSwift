...
### Breaking Changes
- **Major API Rename**: Aligned NDKSwift naming conventions with ndk-core (TypeScript reference implementation)
  - Renamed `observe()` method to `subscribe()` across all APIs
  - Renamed `NDKDataSource<T>` class to `NDKSubscription<T>`
  - Renamed `InternalSubscription` to `NDKSubscriptionCoordinator`
  - Renamed `DataRequirement` to `NDKSubscriptionRequirement`
  - Renamed `NDKDataRequirementManager` to `NDKSubscriptionManager`
  - Renamed `NDKRelaySubscriptionGroup` to `NDKRelaySubscription`
  - All file names updated to match new class names
  
### Migration Guide
- Change all calls from `ndk.observe()` to `ndk.subscribe()`
- Replace `NDKDataSource` type references with `NDKSubscription`
- Update any imports that referenced the old file names

### Why This Change?
This major version bump aligns NDKSwift's naming with ndk-core, making it easier for developers to work across different NDK implementations. The new naming better reflects the actual functionality - we're subscribing to Nostr events, not just observing them.
...
- Fixed broken subscription grouping feature by implementing missing properties:
  - Added `isGroupable`, `groupableDelay`, and `groupableDelayType` properties to NDKSubscriptionCoordinator
...
### Changed
- **BREAKING**: `NDKSubscriptionManager.registerRequirement()` now returns relay updates stream
...
### Changed
- Added `@preconcurrency` attribute to `NDKSubscriptionProtocol` conformances to fix Swift 6 warnings
...
- `NDKRelaySubscription` for grouping subscriptions with the same fingerprint
...
- NDKSubscriptionRequirement simplified to focus on deduplication and relay management
- NDKSubscriptionCoordinator no longer sends REQ messages directly
...
### Changed  
- `NDKSubscriptionCoordinatorManager` no longer maintains a separate set for "universal" subscriptions
...
- `NDKSubscriptionCoordinator.start()` now uses fallback relays as a safety net when relay selection fails
...
- `NDKSubscriptionProtocol` for standardizing SwiftUI data source interfaces
...
- SwiftUI data sources now conform to `NDKSubscriptionProtocol` for consistency
...
### Documentation
- Updated API Reference to emphasize `NDKSubscription` as the primary public subscription API
...
### Changed
- Reduced log verbosity by moving detailed event processing logs from INFO to DEBUG/TRACE level in NDKSubscription
...
### Changed
- Removed TODO comment in NDKSubscriptionManager that was already implemented
...
- Fixed outbox subscriptions not connecting to all NIP-65 relays
  - `NDKSubscriptionCoordinator` now uses `prepareRelays` with `autoConnect: true` to ensure relays are connected before sending REQ messages
...
- Fixed race condition where events arrive before subscription consumers are ready
  - Added `autoStart` parameter to `NDKSubscriptionCoordinatorManager.createSubscription()`
  - `NDKSubscriptionRequirement` now starts subscriptions after setting up event stream consumption
...
- Fixed CLOSE messages being sent to relays that never received REQ
  - `NDKSubscriptionCoordinator` now tracks which relays successfully received REQ messages
...
- Optimized relay connection handling from O(n) to O(1) for subscription replay
  - Added relay-to-subscription mapping in `NDKSubscriptionCoordinatorManager` for direct lookups
...
### Fixed
- Removed unnecessary `await` on non-async property access in `NDKSubscriptionManager`
...
### Fixed
- Fixed thread safety crash in `NDKSubscriptionRequirement` class by converting it to an actor
  ...
  - All mutable state in `NDKSubscriptionRequirement` is now protected by Swift's actor isolation
...
- Custom subscription IDs are now preserved exactly as provided without adding random suffixes
  - When using `NDKSubscription` or `ndk.subscribe()` with a custom `subscriptionId`, the ID is now used as-is
...
- Fixed expert prompt documentation that incorrectly referenced `NDKSubscription` as a public API
  - Clarified that apps should use `ndk.subscribe()` which returns `NDKSubscription`
