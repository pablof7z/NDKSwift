...
public struct FlatBufferBuilder {
    ...
    @usableFromInline var _vtableStorage = VTableStorage()
    /// Flatbuffer data will be written into
    @usableFromInline var _bb: ByteBuffer
    ...
    @inline(__always)
    public mutating func require(table: Offset, fields: [Int32]) {
        for field in fields {
            let start = _bb.capacity &- Int(table.o)
            let startTable = start &- Int(_bb.read(def: Int32.self, position: start))
            let isOkay = _bb.read(
                def: VOffset.self,
                position: startTable &+ Int(field)
            ) != 0
    ...
    @inline(__always)
    public mutating func startTable(with numOfFields: Int) -> UOffset {
        notNested()
        isNested = true
    ...
    public mutating func endTable(at startOffset: UOffset) -> UOffset {
        ...
        for table in _vtables {
            ...
            if len2 != len1 || memcmp(vt1, vt2, Int(len2)) != 0 { continue }
    ...
    @inline(__always)
    @usableFromInline
    mutating func minAlignment(size: Int) {
    ...
    @inline(__always)
    @usableFromInline
    mutating func preAlign(len: Int, alignment: Int) {
    ...
    @inline(__always)
    @usableFromInline
    mutating func preAlign<T: Scalar>(len: Int, type _: T.Type) {
    ...
    @inline(__always)
    @usableFromInline
    mutating func refer(to off: UOffset) -> UOffset {
        let size = MemoryLayout<UOffset>.size
    ...
    @inline(__always)
    @usableFromInline
    mutating func track(offset: UOffset, at position: VOffset) {
    ...
    @inline(__always)
    public mutating func startVector(_ len: Int, elementSize: Int) {
        notNested()
        isNested = true
    ...
    @inline(__always)
    public mutating func endVector(len: Int) -> Offset {
    ...
    @inline(__always)
    public mutating func createVector<T: Scalar>(_ elements: [T]) -> Offset {
    ...
    /// - returns: ``Offset`` of the vector
    @inline(__always)
    public mutating func createVector<T: Scalar>(
        _ elements: [T],
        size: Int
    ) -> Offset {
        let size = size
    ...
    /// - returns: ``Offset`` of the vector
    @inline(__always)
    public mutating func createVector<T: Enum>(_ elements: [T]) -> Offset {
        createVector(elements, size: elements.count)
    }
    ...
    /// - returns: ``Offset`` of the vector
    @inline(__always)
    public mutating func createVector<T: Enum>(
        _ elements: [T],
        size: Int
    ) -> Offset {
        let size = size
    ...
    /// - returns: ``Offset`` of the vector
    @inline(__always)
    public mutating func createVector(ofOffsets offsets: [Offset]) -> Offset {
    ...
    /// - returns: ``Offset`` of the vector
    @inline(__always)
    public mutating func createVector(
    ...
    /// - returns: ``Offset`` of the vector
    @inline(__always)
    public mutating func createVector(ofStrings str: [String]) -> Offset {
        var offsets: [Offset] = []
        for s in str {
    ...
    @inline(__always)
    public mutating func create(string str: String?) -> Offset {
    ...
    /// - returns: ``Offset`` of inserted string
    @inline(__always)
    public mutating func createShared(string str: String?) -> Offset {
        guard let str = str else { return Offset() }
    ...
    ///   - position: The predefined position of the object
    @inline(__always)
    public mutating func add(offset: Offset, at position: VOffset) {
    ...
    @inline(__always)
    @discardableResult
    public mutating func push(element o: Offset) -> UOffset {
    ...
    @inline(__always)
    public mutating func add<T: Scalar>(
        element: T,
        def: T,
        at position: VOffset
    ) {
    ...
    ///   - position: The predefined position of the element
    @inline(__always)
    public mutating func add<T: Scalar>(element: T?, at position: VOffset) {
    ...
    @inline(__always)
    @discardableResult
    public mutating func push<T: Scalar>(element: T) -> UOffset {
        let size = MemoryLayout<T>.size
        preAlign(
            len: size,
            alignment: size
