...
/// Greedy set-cover selector that minimizes total relay connections
/// Uses the greedy set cover algorithm: repeatedly pick the relay covering the most uncovered authors
public struct GreedySetCoverRelaySelector: RelayDiscoverySelectionStrategy {
    private let tracker: any RelayPreferenceProvider

    public init(tracker: any RelayPreferenceProvider) {
        self.tracker = tracker
    }

    public func selectRelaysToConnect(
        discoveredRelays: Set<RelayURL>,
        for authors: Set<String>,
        existingRelays: Set<RelayURL>,
        connectedRelays: Set<RelayURL>,
        ...
        // Build complete relay -> authors map for ALL authors
        var relayToAuthors: [RelayURL: Set<String>] = [:]
        ...
        // Determine which authors are already covered by existing relays
        var coveredAuthors = Set<String>()
        for relay in existingRelays {
            if let covered = relayToAuthors[relay] {
                coveredAuthors.formUnion(covered)
            }
        }

        // Authors that still need coverage
        var uncoveredAuthors = authors.subtracting(coveredAuthors)
        var selectedRelays = Set<RelayURL>()

        // Greedy set cover: repeatedly pick relay with most uncovered authors
        while !uncoveredAuthors.isEmpty {
            // Find relay that covers the most uncovered authors
            var bestRelay: RelayURL?
            var bestNewCoverage = 0
            var bestIsConnected = false

            for (relay, relayAuthors) in relayToAuthors {
                // Skip if already selected or in existing relays
                if selectedRelays.contains(relay) || existingRelays.contains(relay) {
                    continue
                }

                let newCoverage = relayAuthors.intersection(uncoveredAuthors).count
                let isConnected = connectedRelays.contains(relay)

                // Prefer: more coverage, then already-connected
                if newCoverage > bestNewCoverage ||
                   (newCoverage == bestNewCoverage && isConnected && !bestIsConnected) {
                    bestRelay = relay
            ...
            // If no relay can cover any uncovered authors, stop
            guard let relay = bestRelay, bestNewCoverage > 0 else {
                break
            }

            // Add this relay and mark authors as covered
            selectedRelays.insert(relay)
            if let covered = relayToAuthors[relay] {
                uncoveredAuthors.subtract(covered)
            }
        }

        NDKLogger.log(.info, category: .outbox,
                      "🎯 [GreedySetCover] Selected \(selectedRelays.count) relays to cover \(authors.count - uncoveredAuthors.count)/\(authors.count) authors")

        return selectedRelays
    }

    /// Compute the minimum relay set needed to cover all authors with known relays
    /// Returns: (selectedRelays, coverageDetails) where coverageDetails shows marginal gain per relay
    public func computeOptimalCoverage(
        for authors: Set<String>,
        existingRelays: Set<RelayURL> = [],
        connectedRelays: Set<RelayURL> = []
    ) async -> (relays: [RelayURL], coverage: [(relay: RelayURL, newAuthors: Int, cumulative: Int, isConnected: Bool)]) {
        // Build complete relay -> authors map
        var relayToAuthors: [RelayURL: Set<String>] = [:]
        var authorsWithRelays = Set<String>()

        for author in authors {
            if let relayInfo = await tracker.getRelaysSyncFor(pubkey: author, type: .both) {
                let allRelays = relayInfo.readRelays.union(relayInfo.writeRelays)
                if !allRelays.isEmpty {
                    authorsWithRelays.insert(author)
                }
                for relay in allRelays {
                    let normalizedUrl = relay.url.normalizedRelayURL
                    relayToAuthors[normalizedUrl, default: []].insert(author)
        ...
        // Track coverage from existing relays
        var coveredAuthors = Set<String>()
        for relay in existingRelays {
            if let covered = relayToAuthors[relay] {
                coveredAuthors.formUnion(covered)
        ...
        var uncoveredAuthors = authorsWithRelays.subtracting(coveredAuthors)
        var selectedRelays: [RelayURL] = []
        var coverageDetails: [(relay: RelayURL, newAuthors: Int, cumulative: Int, isConnected: Bool)] = []
        ...
        while !uncoveredAuthors.isEmpty {
            ...
            selectedRelays.append(relay)
            if let covered = relayToAuthors[relay] {
                uncoveredAuthors.subtract(covered)
            }

            let cumulative = authorsWithRelays.count - uncoveredAuthors.count
            coverageDetails.append((relay: relay, newAuthors: bestNewCoverage, cumulative: cumulative, isConnected: bestIsConnected))
        }

        return (selectedRelays, coverageDetails)
    }
}
