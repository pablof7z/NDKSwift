...
public struct NDKUIQRCodeGenerator {
    ...
    public static func generate(from string: String, scale: CGFloat = 10) -> UIImage? {
        guard let data = string.data(using: .utf8) else { 
            print("NDKUIQRCodeGenerator: Failed to convert string to UTF8 data")
            return nil 
        }
        ...
        guard let outputImage = filter.outputImage else { 
            print("NDKUIQRCodeGenerator: CIFilter failed to generate output image")
            return nil 
        }
        ...
        guard let cgImage = context.createCGImage(scaledImage, from: scaledImage.extent) else { 
            print("NDKUIQRCodeGenerator: Failed to create CGImage from CIImage")
            return nil 
        }
...
public struct NDKUIQRCodeView: View {
    let content: String
    ...
    public var body: some View {
        Group {
            if let qrImage = NDKUIQRCodeGenerator.generate(from: content) {
                #if os(iOS)
...
public struct NDKUIQRCodeDisplayModifier: ViewModifier {
    let content: String
    let title: String
...
public struct NDKUIQRCodeDisplayView: View {
    let content: String
    let title: String
    @Binding var isPresented: Bool
    @State private var copied = false
    
    public init(content: String, title: String, isPresented: Binding<Bool>) {
        self.content = content
        self.title = title
    ...
    public var body: some View {
        NavigationStack {
            VStack(spacing: 30) {
                ...
                // QR Code
                NDKUIQRCodeView(content: content)
...
public extension View {
    ...
    func ndkQRCodeSheet(for content: String, title: String, isPresented: Binding<Bool>) -> some View {
        modifier(NDKUIQRCodeDisplayModifier(content: content, title: title, isPresented: isPresented))
    }
}