...
public struct NDKUIRelativeTime: View {
    ...
    private var relativeTimeString: String {
        ...
        if elapsed < 31_536_000 {
            let months = Int(elapsed / 2_592_000)
            return months == 1 ? "1mo" : "\(months)mo"
        }

        // Years
        let years = Int(elapsed / 31_536_000)
    ...
    struct NDKUIRelativeTime_Previews: PreviewProvider {
        static var previews: some View {
            VStack(spacing: 16) {
                ...
                // 1 week ago
                NDKUIRelativeTime(timestamp: Date.currentNostrTimestamp - 604_800)

                // 2 months ago
                NDKUIRelativeTime(timestamp: Date.currentNostrTimestamp - 5_184_000)
            }
            .padding()
        }
    }
