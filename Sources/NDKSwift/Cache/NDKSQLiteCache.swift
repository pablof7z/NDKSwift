...
public actor NDKSQLiteCache: NDKCache {
    ...
    /// Decode keyset from database row
    private nonisolated func decodeKeyset(from row: Row) -> CashuSwift.Keyset? {
    ...
    /// Insert a single keyset into the database
    private nonisolated func performKeysetInsert(_ keyset: CashuSwift.Keyset, mintUrl: String, in db: Database) throws {
    ...
    public func saveKeyset(_ keyset: CashuSwift.Keyset, mintUrl: String) async throws {
        ...
        try await dbQueue.write { db in
            ...
            try self.performKeysetInsert(keyset, mintUrl: mintUrl, in: db)
        }
    }
        ...
        do {
            return try await dbQueue.write { db in
                ...
                if let row = try Row.fetchOne(db, sql: "SELECT json FROM keysets WHERE keyset_id = ?", arguments: [id]) {
                    return self.decodeKeyset(from: row)
                }
                return nil
            }
        } catch {
    ...
    public func getKeysets(mintUrl: String) async -> [CashuSwift.Keyset] {
        do {
            return try await dbQueue.read { db in
                ...
                return rows.compactMap { self.decodeKeyset(from: $0) }
            }
        } catch {
