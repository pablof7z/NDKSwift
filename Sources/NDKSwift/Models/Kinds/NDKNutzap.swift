...
public struct NDKNutzap {
    ...
    public var proofs: [CashuProof] {
        get {
            ...
            guard let data = event.content.data(using: .utf8),
                  let proofs = try? JSONDecoder().decode([CashuProof].self, from: data) else {
                return []
            }
            return proofs
        }
    ...
    public mutating func setTarget(_ target: NDKEventPointer) {
        ...
        // Add new target tag
        switch target {
        case .event(let id, let relayURL):
    ...
    public func publish(on relaySet: NDKRelaySet) async throws {
