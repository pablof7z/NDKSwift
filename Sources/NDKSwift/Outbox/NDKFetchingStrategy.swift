    ...
    private func executeFetch(operation: FetchOperation) async throws -> [NDKEvent] {
        ...
        await withTaskGroup(of: FetchResult.self) { group in
            ...
            for await result in group {
                switch result {
                case .success(let events, let relayURL):
                    successfulRelays += 1
                ...
                case .failure(let error):
                    ...
                    if let fetchError = error as? FetchError,
                       case .relayError(let relayURL, _) = fetchError {
...
enum FetchError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .relayError(let url, let message):
            return "Relay error at \(url): \(message)"
        case .insufficientRelays(let required, let successful):
