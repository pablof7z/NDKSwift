...
final class SubscriptionGroupingTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        // Create NDK with a mock relay
        ndk = NDK()
        mockRelay = MockRelay(url: "wss://test.relay")
        try await ndk.addRelay(mockRelay)
    }
    ...
    func testMultipleSubscriptionsWithinGroupingWindow() async throws {
        ...
        for i in 0..<5 {
            let filter = NDKFilter(kinds: [1], authors: ["author\(i)"])
        ...
        let relaySubscriptions = await mockRelay.activeSubscriptions
        ...
        if let mergedSub = relaySubscriptions.values.first,
    ...
    func testSubscriptionsWithDifferentKindsNotGrouped() async throws {
        ...
        for i in 0..<3 {
            let filter = NDKFilter(kinds: [EventKind(i + 1)], authors: ["testauthor"])
            let subscription = await ndk.subscribe(
                filters: [filter],
                options: NDKSubscriptionOptions()
            )
    ...
    func testCloseOnEoseSubscriptionsNotGrouped() async throws {
        ...
        let filter1 = NDKFilter(kinds: [1], authors: ["author1"])
        var options1 = NDKSubscriptionOptions()
        options1.closeOnEose = false
        let sub1 = await ndk.subscribe(filters: [filter1], options: options1)
        subscriptions.append(sub1)
        
        // CloseOnEose subscription
        let filter2 = NDKFilter(kinds: [1], authors: ["author2"])
        var options2 = NDKSubscriptionOptions()
        options2.closeOnEose = true
        let sub2 = await ndk.subscribe(filters: [filter2], options: options2)
        subscriptions.append(sub2)
        
        // Another regular subscription
        let filter3 = NDKFilter(kinds: [1], authors: ["author3"])
        var options3 = NDKSubscriptionOptions()
        options3.closeOnEose = false
        let sub3 = await ndk.subscribe(filters: [filter3], options: options3)
