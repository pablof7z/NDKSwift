...
final class CacheSingleSourceTruthTests: XCTestCase {
    ...
    func testAllEventsFlowThroughCache() async throws {
        ...
        // The cache will have 3 registrations:
        // 1. NDKPool's initial blocked relay fetch (kinds: [10006], limit: 1)
        // 2. Our test's data source (kinds: [1], limit: 10)
        // 3. NDKPool's ongoing blocked relay monitoring (kinds: [10006], limit: 0)
        XCTAssertEqual(registrationCount, 3, "Expected 3 registrations (2 from NDKPool + 1 from test)")
