...
class FollowPackMockRelay: RelayProtocol, @unchecked Sendable {
    ...
    func addSubscriptionId(_: String) async {}
    func removeSubscription(byId _: String) async {}
    func getSignatureStats() async -> NDKRelaySignatureStats { NDKRelaySignatureStats() }
    func updateSignatureStats(_: @Sendable (inout NDKRelaySignatureStats) -> Void) async {}
    func observeConnectionState(_: @escaping @Sendable (NDKRelayConnectionState) -> Void) async {}
    func publish(_: NDKEvent) async throws -> (success: Bool, message: String?) {
        return (true, nil)
    }
}
