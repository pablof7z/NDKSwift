...
final class EphemeralEventFilteringTests: XCTestCase {
    ...
    func testSQLiteCacheQueryEventsExcludesEphemeral() async throws {
        ...
        let events = [
            try await createSignedEvent(kind: 1, content: "note 1"),
            try await createSignedEvent(kind: 20001, content: "ephemeral 1"),
            try await createSignedEvent(kind: 1, content: "note 2"),
            try await createSignedEvent(kind: 25555, content: "ephemeral 2"),
            try await createSignedEvent(kind: 30000, content: "parameterized"),
            try await createSignedEvent(kind: 29999, content: "ephemeral max")
        ]
