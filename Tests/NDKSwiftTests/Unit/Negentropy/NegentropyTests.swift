...
final class NegentropyTests: XCTestCase {
    ...
    func testNegentropyItemCreation() throws {
        ...
        let timestamp: UInt64 = 1_234_567_890
    ...
    func testCacheRangeQueries() async throws {
        ...
        let baseTime = Timestamp(1_000_000)
        for i in 0 ..< 20 {
    ...
    func testNegentropyReconcilerFlow() async throws {
        ...
        case let .continuing(data, _, _):
            ...
            switch response2 {
            case let .terminated(haveIds1, needIds1, isDone):
    ...
    func testNegentropyReconcilerLargeSetReconciliation() async throws {
        ...
        while true {
            ...
            switch response {
            case let .continuing(data, haveIds, needIds):
                totalHave += haveIds.count
                totalNeed += needIds.count
                currentData = data
                isReconciler1Turn.toggle()

            case let .terminated(haveIds, needIds, _):
                totalHave += haveIds.count
                totalNeed += needIds.count
    ...
    private func createTestEvents(ndk _: NDK, count: Int, baseId: Int = 0, baseTime: Int = 1_000_000) async throws -> [NDKEvent] {
        var events: [NDKEvent] = []
